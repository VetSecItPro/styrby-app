 ...
 * }
 *
 * @error 400 { error: 'Missing referral code' }
 ...
 * @body {
 *   referralCode: string,
 *   referrerIp?: string  // passed from the client cookie attribution
 * }
 *
...
export async function GET(request: NextRequest) {
  ...
  // WHY custom config (20 req/min per IP): The GET endpoint is public and
  // called from the landing page referral banner. 20/min is generous enough
  // for normal use while blocking rapid enumeration of referral codes.
  const rl = await rateLimit(request, { windowMs: 60_000, maxRequests: 20 }, `referral-validate:${ip}`);
  if (!rl.allowed) {
  ...
  try {
    ...
    // Record the click in referral_events (idempotent — OK to have multiple clicks)
 ...
 * Called from the signup callback when a referral cookie is present.
 */
export async function POST(request: NextRequest) {
  ...
  const rl = await rateLimit(request, { windowMs: 60_000, maxRequests: 5 }, `referral-attribute:${user.id}`);
  if (!rl.allowed) {
    return rateLimitResponse(rl.retryAfter ?? 60);
  }
  ...
  try {
    ...
    if (referrer.id === user.id) {
      // Mark existing click event as rejected
      await adminSupabase
    ...
    const isDomainMatch =
      ...
      // Exempt common public providers (gmail, outlook, yahoo, etc.)
      !['gmail.com', 'googlemail.com', 'outlook.com', 'hotmail.com', 'yahoo.com', 'icloud.com', 'me.com', 'protonmail.com'].includes(emailDomain);
